...
from .. import initializations_scol
...
class LinDenseTrainable(Layer):
    ...
    def __init__(self, output_dim, init='identity_vstacked',
        ...
        if self.input_dim:
            kwargs['input_shape'] = (self.input_dim,)
        super(LinDenseTrainable, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.W2 = self.add_weight((input_dim, self.output_dim),
                                 initializer=self.init,
                                 name='{}_W'.format(self.name),
                                 regularizer=self.W_regularizer,
                                 constraint=self.W_constraint)
    ...
    def call(self, x, mask=None):
        ...
        self.W /= (K.epsilon() + K.sum(self.W , axis=0, keepdims=True))
        output = K.dot(x, self.W)
        if self.bias:
            output += self.b
        return self.activation(output)
    ...
    def get_config(self):
        config = {'output_dim': self.output_dim,
                  ...
                  'bias': self.bias,
                  'input_dim': self.input_dim}
        base_config = super(LinDenseTrainable, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
