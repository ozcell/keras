...
for i in range(5):
    ...
    results.append([])
    results[i].append(np.asarray(history.history.get('loss')))
    results[i].append(np.asarray(history.history.get('val_loss')))
    results[i].append(np.asarray(history.history.get('acc')))
    results[i].append(np.asarray(history.history.get('val_acc')))

    model.save_weights('weights_2048x3_100_050_'+ str(i+1) + '_of_5.h5', overwrite=True)

f = open('history_2048x3_100_050_of_5.save', 'wb')
